...
Обектната система на Perl 5 макар и гъвкава е минимална. Може да се
използва за направата на големи неща, но предлага малко помощ при
извършването на някои основни задачи. I<Moose> е пълноценна обектна
система за Perl 5N<Вижте C<perldoc Moose::Manual> за повече
информация>. Той улеснява програмиста в създаването на обекти, а също
така дава и много нови възможности взаимствани от езици като
Smalltalk, Common Lisp и Perl 6. Moose надгражда над обектната система
на езика и в момента е най-добрият начин за разработката на обектно
ориентирани програми за Perl 5.
...
Един обект в Moose е конкретна инстанция на някой I<клас>, който
представлява шаблон описващ данни и поведение специфично за
обекта. Класовете използват пакети (L<packages>) за да предоставят
...
Класът C<Cat> I<привидно> не прави нищо, но Moose има нужда само от
толкова малко код за да направи клас. За да се направят обекти (или
I<инстанции>) на класа C<Cat> трябва да се използват следните конструктори:
...
Стрелкичката се използва както за обръщане към стойността, сочена от
референция, така и за да се извика метод на обект или клас.
...
I<Методът> е функция, която е свързана с някой клас. Така както
функциите принадлежат към именовани пространства, така и методите
принадлежат на класове, но с две разлики. Първата е, че методът винаги
работи върху този, който го извиква. Извиквайки методът C<new()> върху
C<Cat> прави същото като да изпрати съобщение до класа C<Cat>. Името
на класа, C<Cat>, е нещото, което извиква C<new()>. Когато някой метод
бъде извикан върху обект, този обект е всъщност е викащият:
